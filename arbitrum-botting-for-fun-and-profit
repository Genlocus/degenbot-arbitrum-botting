...
BROWNIE_NETWORK = "changeme"
BROWNIE_ACCOUNT = "changeme"
...
ARB_CONTRACT_ADDRESS = "changeme"
WETH_ADDRESS = "0x82aF49447D8a07e3bd95BD0d56f35241523fBab1"

MIN_PROFIT_ETH = int(0.001 * 10**18)
...
async def main():
    ...
    for sig in signals:
        asyncio.get_event_loop().add_signal_handler(
            sig, functools.partial(shutdown, sig)
        )
...
async def activate_arbs():
    ...
    _inactive = list(inactive_arbs.values())
    _active_unprocessed = list(active_arbs.values())
    start_position = None
    ...
    while True:
        ...
        # Activate arbs from the inactive set (identified by the block watcher) first.
        # If none are available, process a sample from the inactive_arbs dict
        if arbs_to_activate:
            arbs_to_process = list(set(arbs_to_activate))
            arbs_to_activate.clear()
        elif _active_unprocessed:
            i = 0
            end = min(len(_active_unprocessed), 25)
            while i < end:
                arb_helper = _active_unprocessed.pop()
            ...
            end_position = min(len(_inactive), start_position + 25)

            arbs_to_process = _inactive[start_position:end_position]

            # if we've reached the end, rebuild the list and restart at the first position
            if end_position == len(_inactive):
                _inactive = list(inactive_arbs.values())
                start_position = None

        for arb_helper in arbs_to_process:
            # arbs can be submitted to the queue multiple times,
            # so skip any that have already been activated and no
            # longer part of the inactive_arbs dict
            if arb_helper.id not in inactive_arbs:
                continue
            ...
            try:
                ...
                print(f"(activate_arbs) (catch-all): {e}")
                continue
            else:
                ...
                if await asyncio.get_running_loop().run_in_executor(
                    None, test_onchain_arb_gas, arb_helper
                ):
...
async def execute_arb_with_rpc(
    arbitrage: bot.arbitrage.base.Arbitrage,
    nonce: int,
) -> bool:
    if VERBOSE_TIMING:
...
async def load_arbs():
    ...
    for token_address in unique_tokens:
        ...
        try:
            token_helper = bot.manager.token_manager.Erc20TokenHelperManager().get_erc20token(
                token_address,
                silent=True,
                min_abi=True,
            )

        except bot.exceptions.ManagerError as e:
            BLACKLISTED_TOKENS.append(token_address)
            print(f"Blacklisted token {token_address}: {e}")
        except Exception as e:
            print(e)
            print(type(e))
        else:
            ...
            degenbot_token_helpers[token_address] = token_helper

    print(
        f"Built {len(degenbot_token_helpers)} tokens in {time.time() - start :.2f}s"
    )

    with open("arbitrum_blacklisted_tokens.json", "w") as file:
    ...
    for pool_address in unique_pool_addresses:
        ...
        try:
            if pool_type == "UniswapV2":
                ...
                pool_helper = (
                    bot.uniswap.v2.liquidity_pool.CamelotLiquidityPool(
                        address=pool_address,
                        tokens=[token0_obj, token1_obj],
                        update_method="external",
                        silent=True,
...
async def process_onchain_arbs():
    ...
    while True:
        ...
        arb_helper = active_arbs_to_check.popleft()

        try:
            await asyncio.sleep(0)
            if arb_helper.auto_update(silent=True):
                result = await asyncio.get_running_loop().run_in_executor(
                    None, arb_helper.calculate_arbitrage
                )
            else:
                continue
        except asyncio.exceptions.CancelledError:
            ...
            # print(f"(process) (ArbitrageError): {e}")
            if VERBOSE_ACTIVATION:
                print(f"(process) (ArbitrageError): {e}")
        except Exception as e:
            print(f"(process_onchain_arbs) catch-all: {e}")
            continue
...
def shutdown(sig):
    ...
    for task in [
        t for t in asyncio.all_tasks() if not (t.done() or t.cancelled())
    ]:
        task.cancel()
...
async def sweep():
    try:
        while True:
            ...
            # process all profitable arbs until one is successfully executed,
            # then clear it and break the loop
            for arb_helper in profitable_arbs:
                print(f"executing {arb_helper}")
...
async def watch_events():
    ...
    def process_burn_event(message: dict) -> None:
        ...
        try:
            ...
            try:
                ...
                active_arbs_to_check.extendleft(
                    [
                        arb
                        for arb in active_arbs.values()
                ...
                active_arbs_to_check.extendleft(
                    [
                        ...
                        if v3_pool_helper is arb.swap_pools[1]
                        # if v3_pool_helper in arb.swap_pools
                    ]
                )
                arbs_to_activate.extendleft(
                    [
                        arb
                        for arb in inactive_arbs.values()
                        if len(arb.swap_pools) == 2
    ...
    def process_sync_event(message: dict) -> None:
        ...
        try:
            v2_pool_helper: bot.LiquidityPool = degenbot_lp_helpers[
                event_address
            ]
        except KeyError:
            return
            ...
            try:
                ...
                active_arbs_to_check.extendleft(
                    [
                        ...
                        for arb in active_arbs.values()
                        if len(arb.swap_pools) == 3
                        if v2_pool_helper is arb.swap_pools[1]
                        # if v2_pool_helper in arb.swap_pools
                    ]
                )
                arbs_to_activate.extendleft(
        ...
        while True:
            ...
            if status_live and event_queue:
                ...
                # skip checking for the signature in TOPICS and simply continue on untracked events
                    ...
                    if VERBOSE_PROCESSING:
                        print(
                            f"processed {TOPICS[topic0]['name']} event - {len(event_queue)} remaining"
                        )
...
inactive_arbs = dict()
active_arbs = dict()
arb_simulations = dict()
active_arbs_to_check = deque()
arbs_to_activate = deque()
